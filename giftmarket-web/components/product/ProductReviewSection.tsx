...
export default function ProductReviewSection({ productId }: { productId: number }) {
  const [data, setData] = useState<ReviewPage | null>(null); const [page, setPage] = useState(0); const [loading, setLoading] = useState(true);
  ...
  // 페이지 변경 직후 기존 집계가 현재 조회 결과처럼 보이지 않게 한다.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { let active=true; setLoading(true); setError(""); getProductReviews(productId,page).then(v=>{if(active)setData(v);}).catch(e=>{if(active)setError(e instanceof Error?e.message:"리뷰를 불러오지 못했습니다.");}).finally(()=>{if(active)setLoading(false);}); return()=>{active=false}; }, [productId,page]);
  return <section id="product-reviews" className="product-review-section">
    <header><div><h2>리뷰</h2><p>구매 확정한 고객의 실제 구매 후기입니다.</p></div><strong><span aria-hidden="true">★</span> {loading || error || !data ? "-" : data.averageRating.toFixed(1)} <small>리뷰 {loading || error || !data ? "-" : data.reviewCount}개</small></strong></header>
    {error ? <p className="product-review-empty">{error}</p> : loading || !data ? <p className="product-review-empty">리뷰를 불러오는 중입니다.</p> : data.reviews.length===0 ? <p className="product-review-empty">아직 작성된 리뷰가 없습니다.</p> : <>
