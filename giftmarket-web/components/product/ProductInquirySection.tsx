...
export default function ProductInquirySection({ productId }: { productId: number }) {
  ...
  const load = useCallback(async () => { try { setLoading(true); setError(""); const nextResult = await getProductInquiries(productId, page); if (page > 0 && (nextResult.totalPages === 0 || page >= nextResult.totalPages)) { setPage(Math.max(0, nextResult.totalPages - 1)); return; } setResult(nextResult); } catch (e) { setError(e instanceof Error ? e.message : "상품 문의를 불러오지 못했습니다."); } finally { setLoading(false); } }, [page, productId]);
  useEffect(() => {
  ...
  return <section id="product-inquiries" className="product-inquiry-section">
    <header><div><p>PRODUCT Q&amp;A</p><h2>상품 문의 <span>{loading || error || !result ? "-" : result.totalElements}</span></h2></div><button type="button" onClick={startCreate}>문의 작성</button></header>
