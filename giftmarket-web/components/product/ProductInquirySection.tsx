...
export default function ProductInquirySection({ productId }: { productId: number }) {
  ...
  return <section id="product-inquiries" className="product-inquiry-section">
    ...
    {result && result.totalPages > 1 && <div className="product-inquiry-pagination"><button disabled={result.first} onClick={() => setPage((v) => v - 1)}>이전</button><span>{result.page + 1} / {result.totalPages}</span><button disabled={result.last} onClick={() => setPage((v) => v + 1)}>다음</button></div>}
