...
export default function SellerInquiriesPage() {
  ...
  return <main className="seller-inquiries-page"><div className="seller-inquiries-container"><header className="seller-inquiries-header"><div><p>PRODUCT Q&amp;A</p><h1>상품 문의 관리</h1></div><strong>{result?.totalElements ?? 0}건</strong></header><div className="seller-inquiry-filters">{filters.map((f) => <button key={f.label} className={status === f.value ? "active" : ""} onClick={() => { setStatus(f.value); setPage(0); }}>{f.label}</button>)}</div>{loading ? <div className="seller-inquiry-state">문의 목록을 불러오고 있습니다.</div> : error ? <div className="seller-inquiry-state"><p>{error}</p><button onClick={() => void load()}>다시 시도</button></div> : result && result.inquiries.length ? <><table className="seller-inquiry-table"><thead><tr><th>문의번호</th><th>상품</th><th>제목</th><th>작성자</th><th>작성일</th><th>상태</th><th>관리</th></tr></thead><tbody>{result.inquiries.map((i) => <tr key={i.id}><td>#{i.id}</td><td>{i.productName}</td><td>{i.isPrivate ? "🔒 " : ""}{i.title}</td><td>{i.writerName}</td><td>{new Date(i.createdAt).toLocaleDateString("ko-KR")}</td><td><span className={`seller-inquiry-status ${i.status === "ANSWERED" ? "answered" : ""}`}>{i.status === "ANSWERED" ? "답변 완료" : "답변 대기"}</span></td><td><Link href={`/seller/inquiries/${i.id}`}>상세</Link></td></tr>)}</tbody></table>{result.totalPages > 1 && <div className="seller-inquiry-pagination"><button disabled={result.first} onClick={() => setPage((v) => v - 1)}>이전</button><span>{result.page + 1} / {result.totalPages}</span><button disabled={result.last} onClick={() => setPage((v) => v + 1)}>다음</button></div>}</> : <div className="seller-inquiry-state">해당 상태의 상품 문의가 없습니다.</div>}</div></main>;
}
