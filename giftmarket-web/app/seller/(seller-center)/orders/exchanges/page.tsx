...
export default function SellerExchangesPage() {
  ...
  useEffect(() => { if (!initialized) return; if (!authenticated || !user) { router.replace("/login"); return; } if (user.role !== "SELLER" && user.role !== "ADMIN") { router.replace("/seller"); return; } const id = window.setTimeout(() => void load(), 0); return () => window.clearTimeout(id); }, [authenticated, initialized, load, router, user]);
