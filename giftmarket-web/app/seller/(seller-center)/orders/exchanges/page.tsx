...
export default function SellerExchangesPage() {
  ...
  useEffect(() => { if (!initialized) return; if (!authenticated || !user) { router.replace(getLoginRedirectUrl()); return; } const id = window.setTimeout(() => void load(), 0); return () => window.clearTimeout(id); }, [authenticated, initialized, load, router, user]);
