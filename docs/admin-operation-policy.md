...
구현된 1차 범위는 다음과 같다. 아래 번호는 도입 순서의 기록이지 미구현 계획이 아니다.
...
현재 Admin은 판매자 신청 승인·거절, 회원 정지·해제, 판매자 판매정지·해제, 상품 강제 숨김·해제, 주문·취소·반품·교환 조회, 알림과 정산 관리를 제공한다. 정산은 조회·수동 생성·`READY → ON_HOLD → READY`·`READY → CONFIRMED`를 지원한다. `CONFIRMED`는 금액 확정이며 송금이 아니다.

Seller `SUSPENDED` 강제 정지와 Admin 취소·환불·반품·교환 중재는 현재 범위에 포함하지 않는다. 실제 CS·분쟁 요구사항과 미처리 거래 대행 정책이 확정된 후 기존 workflow를 재사용하는 별도 command 구조로 확장한다. 정산 자동 생성은 기존 `SettlementGenerationService`를 호출하는 Scheduler로 별도 구현한다.
