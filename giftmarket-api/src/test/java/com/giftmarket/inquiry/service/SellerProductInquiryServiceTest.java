...
class SellerProductInquiryServiceTest {
    ...
    @BeforeEach void setup(){ service=new SellerProductInquiryService(inquiries,answers,sellers); given(sellers.findByUserId(1L)).willReturn(Optional.of(seller)); given(seller.getStatus()).willReturn(SellerStatus.ACTIVE); given(seller.getId()).willReturn(9L); }
