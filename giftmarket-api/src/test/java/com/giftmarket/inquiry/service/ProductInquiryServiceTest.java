...
class ProductInquiryServiceTest {
    ...
    @Test void hiddenOrDeletedProductCreateFails(){ given(products.findByIdAndStatusInAndDeletedAtIsNull(eq(10L),any())).willReturn(Optional.empty()); assertThatThrownBy(() -> service.create(1L,10L,new ProductInquiryRequest("a","b",false))).isInstanceOf(ProductInquiryException.class); }
    ...
    private void visible(){ given(products.findByIdAndStatusInAndDeletedAtIsNull(eq(10L),any())).willReturn(Optional.of(product)); }
