...
class ProductInquiryServiceTest {
    ...
    @Test void hiddenOrDeletedProductCreateFails(){ given(products.findByIdAndStatusInAndAdminHiddenFalseAndSellerStatusAndDeletedAtIsNull(eq(10L),any(),eq(SellerStatus.ACTIVE))).willReturn(Optional.empty()); assertThatThrownBy(() -> service.create(1L,10L,new ProductInquiryRequest("a","b",false))).isInstanceOf(ProductInquiryException.class); }
