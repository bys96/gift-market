...
private String normalize(String v){return v==null||v.isBlank()?null:v.trim();}private void admin(Long id){if(id==null)throw new AuthenticationException("인증이 필요합니다.");var u=users.findById(id).orElseThrow(()->new AuthenticationException("사용자를 찾을 수 없습니다."));if(!u.getRole().isAdmin())throw new AuthenticationException("관리자 권한이 필요합니다.");}
}
