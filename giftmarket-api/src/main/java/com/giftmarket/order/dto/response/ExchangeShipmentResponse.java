...
public record ExchangeShipmentResponse(
        Long shipmentId, ShipmentType type, ShipmentStatus status,
        String shippingCompany, String trackingNumber,
        LocalDateTime shippedAt, LocalDateTime deliveredAt
) {
    public static ExchangeShipmentResponse from(Shipment shipment) {
        if (shipment == null) return null;
        return new ExchangeShipmentResponse(
